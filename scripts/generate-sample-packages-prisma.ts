 ...
 * - 5 Chain Packages
 *
 * Updated to use Prisma ORM
 */

const prisma = new PrismaClient();

async function main() {
  console.log("🚀 Starting sample data generation (Prisma)...\n");
  ...
  try {
    ...
    for (const pkg of vectorPkgs) {
      await prisma.vectorPackage.create({ data: pkg });
    ...
    for (const pkg of memoryPkgs) {
      await prisma.memoryPackage.create({ data: pkg });
      console.log(`  ✓ ${pkg.name}`);
    }
    ...
    for (const pkg of chainPkgs) {
      await prisma.chainPackage.create({ data: pkg });
      console.log(`  ✓ ${pkg.name}`);
    }
    ...
    console.log(`\n🎯 Next steps:`);
    console.log(`  1. Visit http://localhost:3000/vector-packages to see Vector Market`);
    console.log(`  2. Visit http://localhost:3000/memory-packages to see Memory Market`);
    console.log(`  3. Visit http://localhost:3000/chain-packages to see Chain Market`);
  } catch (error) {
