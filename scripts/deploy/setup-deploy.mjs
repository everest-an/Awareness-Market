...
async function main() {
  console.log('╔═══════════════════════════════════════════════════════════════╗');
  console.log('║         MemoryNFT 部署助手 (Polygon Amoy 测试网)            ║');
  console.log('╚═══════════════════════════════════════════════════════════════╝\n');
  ...
  switch(choice.trim()) {
    case '1': {
      ...
      try {
        ...
        // 写入 .env.local
        const envContent = `# Polygon Amoy Testnet Configuration
...
`;
        ...
        console.log('  2. 选择 Polygon Amoy');
        console.log('  3. 输入地址:', wallet.address);
        console.log('  4. 获得测试币后，运行: npm run deploy:amoy');
...
`;
    ...
    case '3': {
      ...
      try {
        ...
        lines.forEach(line => {
          ...
          } else if (line.startsWith('AMOY_RPC_URL')) {
            const url = line.split('=')[1];
            console.log('AMOY_RPC_URL:', url || '❌ 未设置');
    ...
    case '4': {
      ...
      const deploy = spawn('npm', ['run', 'deploy:amoy'], {
        stdio: 'inherit',
        shell: true
      });
