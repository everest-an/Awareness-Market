...
import { mysqlTable, varchar, text, int, bigint, timestamp, json, index } from 'drizzle-orm/mysql-core';
...
export const tokenBoundAccounts = mysqlTable('token_bound_accounts', {
  ...
  salt: int('salt').notNull().default(0),
  
  // Deployment info
  implementationAddress: varchar('implementation_address', { length: 42 }).notNull(),
  deployed: int('deployed').notNull().default(0), // 0 = not deployed, 1 = deployed
...
export const tbaTransactions = mysqlTable('tba_transactions', {
  id: int('id').primaryKey().autoincrement(),
...
export const memoryProvenance = mysqlTable('memory_provenance', {
  id: int('id').primaryKey().autoincrement(),
  ...
  contributionPercent: int('contribution_percent').notNull(), // 0-100
  
  // Royalty info
  royaltyPercent: int('royalty_percent').notNull(), // Percentage of child's revenue
...
export const agentCreditScores = mysqlTable('agent_credit_scores', {
  id: int('id').primaryKey().autoincrement(),
  ...
  // Credit metrics
  creditScore: int('credit_score').notNull().default(500), // 300-850 (like FICO)
  ...
  totalMemoriesCreated: int('total_memories_created').notNull().default(0),
  totalMemoriesSold: int('total_memories_sold').notNull().default(0),
...
export const creditScoreHistory = mysqlTable('credit_score_history', {
  ...
  scoreDelta: int('score_delta').notNull(),
