...
- **修复**: 使用 Prisma `sql`/`$queryRaw` 模板字符串
...
// Line 429, 588
Conversion of raw query result to InsertResult may be a mistake
...
1. **数据库依赖** (29个测试跳过):
   ...
   - api-key.test.ts - 需要PostgreSQL
   - blog.test.ts - 需要PostgreSQL
   - vector-invocation.test.ts - 需要PostgreSQL
