...
export default function PackageDetail() {
  ...
  const { data: purchases } = trpc.packages.myPurchases.useQuery();
            ...
            <Card className="p-8 bg-slate-900/50 border-slate-800">
              ...
              <div className="grid grid-cols-4 gap-4">
                <div className="text-center p-3 bg-slate-800/50 rounded">
                  <div className="text-2xl font-bold text-white">{pkg.downloads || 0}</div>
                ...
                <div className="text-center p-3 bg-slate-800/50 rounded">
                  <div className="text-2xl font-bold text-white">{pkg.reviewCount || 0}</div>
                  <div className="text-xs text-slate-400">Reviews</div>
                </div>
                <div className="text-center p-3 bg-slate-800/50 rounded">
                  <div className="text-2xl font-bold text-white">v{pkg.version || '1.0.0'}</div>
            ...
            <Card className="p-6 bg-slate-900/50 border-slate-800">
              ...
              <div className="flex items-center justify-between p-4 bg-slate-800/30 rounded">
                <div className="text-center">
                  ...
                  <div className="text-lg font-mono text-white">{pkg.sourceModel}</div>
                </div>
                ...
                <div className="text-center">
                  ...
                  <div className="text-lg font-mono text-white">{pkg.targetModel}</div>
                </div>
              </div>
            </Card>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-slate-800/30 rounded">
                  ...
                  <div className="text-2xl font-bold text-white">
                    {pkg.epsilon ? Number(pkg.epsilon).toFixed(4) : '0.0000'}
                  </div>
                ...
                <div className="p-4 bg-slate-800/30 rounded">
                  ...
                  <div className="text-2xl font-bold text-green-400">
                    {((pkg.informationRetention || 0.85) * 100).toFixed(1)}%
                  </div>
                ...
                {type === 'vector' && (
                  <>
                    <div className="p-4 bg-slate-800/30 rounded">
                      ...
                      <div className="text-xl font-bold text-cyan-400">{pkg.dimension || 768}</div>
                    </div>
                    <div className="p-4 bg-slate-800/30 rounded">
                      <div className="text-sm text-slate-400 mb-1">Category</div>
                      <div className="text-xl font-bold text-white capitalize">{pkg.category || 'nlp'}</div>
                    </div>
                  </>
                )}
                {type === 'memory' && (
                  <>
                    <div className="p-4 bg-slate-800/30 rounded">
                      ...
                      <div className="text-xl font-bold text-purple-400">{pkg.tokenCount || 0}</div>
                    </div>
                    <div className="p-4 bg-slate-800/30 rounded">
                    ...
                    {pkg.contextDescription && (
                      <div className="col-span-2 p-4 bg-slate-800/30 rounded">
                        ...
                        <div className="text-white">{pkg.contextDescription}</div>
                      </div>
                    )}
                  </>
                )}
                {type === 'chain' && (
                  <>
                    <div className="p-4 bg-slate-800/30 rounded">
                      ...
                      <div className="text-xl font-bold text-emerald-400">{pkg.stepCount || 0}</div>
                    </div>
                    <div className="p-4 bg-slate-800/30 rounded">
                      <div className="text-sm text-slate-400 mb-1">Problem Type</div>
                      <div className="text-xl font-bold text-white capitalize">
                        {pkg.problemType?.replace('-', ' ') || 'general'}
                      </div>
                    </div>
                    <div className="p-4 bg-slate-800/30 rounded">
              ...
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-full bg-slate-700 flex items-center justify-center">
                  <span className="text-xl text-white">
                    {pkg.creator?.name?.[0] || 'A'}
                  </span>
                </div>
                <div>
                  <div className="text-white font-medium">
                    {pkg.creator?.name || 'Anonymous'}
                  </div>
            ...
            {pkg.tags && (
              <Card className="p-6 bg-slate-900/50 border-slate-800">
                ...
                <div className="flex flex-wrap gap-2">
                  {(typeof pkg.tags === 'string' ? JSON.parse(pkg.tags) : pkg.tags).map((tag: string, i: number) => (
