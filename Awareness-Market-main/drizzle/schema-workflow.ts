...
import { mysqlTable, int, varchar, text, timestamp, decimal, json, mysqlEnum, index, bigint } from "drizzle-orm/mysql-core";
...
export const workflowSessions = mysqlTable("workflow_sessions", {
  ...
  status: mysqlEnum("status", ["active", "completed", "failed"]).notNull(),
  ...
  totalDuration: int("total_duration").default(0).notNull(), // milliseconds
