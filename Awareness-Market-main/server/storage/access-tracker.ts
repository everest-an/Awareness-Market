...
export async function recordPackageAccess(record: AccessRecord): Promise<void> {
  try {
    ...
    console.log(`[AccessTracker] Recorded ${record.accessType} for ${record.packageType}:${record.packageId}`);
  } catch (error) {
    console.error('[AccessTracker] Failed to record access:', error);
...
export async function getAccessFrequency(
  ...
  try {
    ...
    console.error('[AccessTracker] Failed to get access frequency:', error);
    return 0;
  }
}
...
export async function determineDataTemperature(
  ...
  try {
    ...
    console.error('[AccessTracker] Failed to determine temperature:', error);
...
export async function getTierInfo(
  ...
  try {
    ...
    console.error('[AccessTracker] Failed to get tier info:', error);
    return null;
  }
}
...
export async function updateTierAssignment(
  ...
  try {
    ...
    console.log(`[AccessTracker] Updated ${packageType}:${packageId} to ${newTier} (${newBackend})`);
  } catch (error) {
    console.error('[AccessTracker] Failed to update tier:', error);
    throw error;
  }
}
...
export async function getPackagesNeedingMigration(): Promise<Array<{
  ...
  try {
    ...
    console.error('[AccessTracker] Failed to get migration candidates:', error);
    return [];
  }
}
...
export async function getAccessStats(
  ...
  try {
    ...
    console.error('[AccessTracker] Failed to get access stats:', error);
    return {
      totalAccess: 0,
      last7Days: 0,
      last30Days: 0,
...
export async function getHotPackages(limit: number = 10): Promise<TierInfo[]> {
  try {
    ...
    console.error('[AccessTracker] Failed to get hot packages:', error);
    return [];
  }
}
...
export async function getColdPackages(limit: number = 10): Promise<TierInfo[]> {
  try {
    ...
    console.error('[AccessTracker] Failed to get cold packages:', error);
    return [];
  }
}
