 ...
 */
export const packageModeEnum = mysqlEnum('package_mode', ['static', 'latent']);
 ...
 */
export const latentmasVectorPackages = mysqlTable('latentmas_vector_packages', {
  id: int('id').autoincrement().primaryKey(),
  
  // Reference to base vector package
  vectorPackageId: int('vector_package_id').notNull().unique(),
  ...
  packageMode: mysqlEnum('package_mode', ['static', 'latent']).default('static').notNull(),
  
  // Latent Rollout Configuration
  latentSteps: int('latent_steps').default(0).notNull(), // 0-80 steps
  ridgeLambda: decimal('ridge_lambda', { precision: 10, scale: 6 }).default('0.010000').notNull(), // 0.0001-1
  
  // Drift Metrics (Quality indicators during latent rollout)
  driftMax: decimal('drift_max', { precision: 10, scale: 6 }).default('0.000000').notNull(),
  driftAvg: decimal('drift_avg', { precision: 10, scale: 6 }).default('0.000000').notNull(),
  driftDetected: boolean('drift_detected').default(false).notNull(),
  
  // Wa Operator Metadata (Alignment matrix quality)
  waConditionNumber: decimal('wa_condition_number', { precision: 15, scale: 6 }).default('0.000000').notNull(),
  waRank: int('wa_rank').default(0).notNull(),
  waComputedAt: timestamp('wa_computed_at'),
  
  // Latent Data Dimensions
  latentThoughtDim: int('latent_thought_dim').default(0).notNull(),
 ...
 */
export const latentmasMemoryPackages = mysqlTable('latentmas_memory_packages', {
  id: int('id').autoincrement().primaryKey(),
  
  // Reference to base memory package
  memoryPackageId: int('memory_package_id').notNull().unique(),
  ...
  // Latent Rollout Configuration
  latentSteps: int('latent_steps').default(0).notNull(),
  ridgeLambda: decimal('ridge_lambda', { precision: 10, scale: 6 }).default('0.010000').notNull(),
 ...
 */
export const latentmasChainPackages = mysqlTable('latentmas_chain_packages', {
  id: int('id').autoincrement().primaryKey(),
  
  // Reference to base chain package
  chainPackageId: int('chain_package_id').notNull().unique(),
 ...
 */
export const latentSpaceStatusEnum = mysqlEnum('latent_space_status', ['active', 'archived', 'deleted']);
 ...
 */
export const userLatentSpaces = mysqlTable('user_latent_spaces', {
  // Primary key
  id: int('id').autoincrement().primaryKey(),
  ...
  quotaLimit: int('quota_limit').default(10).notNull(), // Default: 10 active spaces per user
